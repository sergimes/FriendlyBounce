...
void Physics::update_positions(std::vector<Player> &players,
    ...
    for(int i = 0; i < n_players; ++i){
        ...
        players[i].modify_vx(players[i].get_vx()*0.995);
        players[i].modify_vy(players[i].get_vy()*0.995);
    }
...
void Physics::player_limits(Player &player, int windowWidth, int windowHeight) {
    ...
    if (position.first + player.get_w() > windowWidth and player.get_vx() > 0) {player.modify_vx(0);}
    if (position.second < 0 and player.get_vy() < 0) {player.modify_vy(0);}
    if (position.second + player.get_h() > windowHeight and player.get_vy() > 0) {player.modify_vy(0);}
}
